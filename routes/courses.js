...
router.post('/:id/enroll',(req,res)=>{
    try{
        ...
        let curr_student=student_data.findIndex(ele=>ele['studentId']==studentId)
        if(curr_student==-1)   throw `No student exists with ID : ${studentId}`

        let course_index=course_data.findIndex(ele=>ele['courseId']==courseId)
        if(course_index==-1)    throw `No course exists with ID : ${courseId}`
        
        if(course_data[course_index]['availableSlots']==0)   throw `No availabe slots for course ID : ${courseId}`
...
router.put('/:id/deregister',(req,res)=>{
    try{
        ...
        let curr_student=student_data.filter(ele=>ele['studentId']==studentId)
        ...
        if(student_index==-1)   throw `Student with ID : ${studentId} not registered in this course with ID : ${courseId}`
